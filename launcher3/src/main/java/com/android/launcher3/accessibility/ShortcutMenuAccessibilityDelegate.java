...
public class ShortcutMenuAccessibilityDelegate extends LauncherAccessibilityDelegate {
    ...
    private static final int DISMISS_NOTIFICATION = com.android.launcher3.R.id.action_dismiss_notification;

    public ShortcutMenuAccessibilityDelegate(Launcher launcher) {
        super(launcher);
        mActions.put(DISMISS_NOTIFICATION, new AccessibilityAction(DISMISS_NOTIFICATION,
                launcher.getText(com.android.launcher3.R.string.action_dismiss_notification)));
    }
    ...
    public boolean performAction(View host, ItemInfo item, int action) {
        if (action == ADD_TO_WORKSPACE) {
            ...
            announceConfirmation(com.android.launcher3.R.string.notification_dismissed);
            return true;
        }
        return false;
    }
