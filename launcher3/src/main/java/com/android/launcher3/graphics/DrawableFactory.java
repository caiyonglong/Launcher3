...
public class DrawableFactory {
        ...
        synchronized (LOCK) {
            if (sInstance == null) {
                sInstance = Utilities.getOverrideObject(DrawableFactory.class,
                        context.getApplicationContext(), com.android.launcher3.R.string.drawable_factory_class);
            }
            return sInstance;
        }
    }
    ...
    protected Path getPreloadProgressPath(Context context) {
        if (Utilities.ATLEAST_OREO) {
            try {
                ...
                Drawable icon = context.getDrawable(com.android.launcher3.R.drawable.adaptive_icon_drawable_wrapper);
                icon.setBounds(0, 0,
    ...
    protected synchronized Bitmap getUserBadge(UserHandle user, Context context) {
        ...
        int badgeSize = res.getDimensionPixelSize(com.android.launcher3.R.dimen.profile_badge_size);
